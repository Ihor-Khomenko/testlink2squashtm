    ...
    try:
        ...
        def parse_suite(suite_element, path_prefix=""):
            ...
            for testcase in testcases:
                ...
                # 2. Deduplication & Uniqueness Force
                # We append a random ID to EVERY test case to bypass the database lock
                unique_suffix = generate_short_id()
                
                # If you want strictly original names, remove `_{unique_suffix}` below,
                # BUT this will cause the Duplicate Error again if the DB isn't clean.
                # I highly recommend keeping it for this import.
                final_tc_name = f"{tc_name}_{unique_suffix}"
                ...
                tc_reference = get_node_text(testcase, 'externalid') if MIGRATE_TESTLINK_ID else ''
